...
async function logCommitStatus(logDir, status, message, changedFiles) {
...
------------------------------------------------
`;
  import_fs.default.appendFileSync(logPath, logEntry, "utf8");
}
async function logPushStatus(logDir, status) {
  const logPath = path.join(logDir, LOG_FILE_NAME);
  const now = (/* @__PURE__ */ new Date()).toLocaleString();
  const logEntry = `Date/Time: ${now}
Status: ${status}
...
async function pushCode(remote, branch, logDir) {
  try {
...
async function commitCode(timeGapInMinutes, logDir) {
  try {
...
async function instantCommitCode(logDir) {
  try {
    const commitMessage = await generateCommitMessage();
    const hasChanges = await checkChanges();
    const changedFiles = await git.diff(["--name-only"]);
...
function activate(context) {
  ...
  const disposable = vscode.commands.registerCommand("codePusher.startCommittingCode", async () => {
    ...
    if (!import_fs.default.existsSync(logDir)) {
      import_fs.default.mkdirSync(logDir);
    }
    ...
    timeGapInMinutes = vscode.workspace.getConfiguration().get("codePusher.timeGapInMinutes") || 30;
    interval = setInterval(async () => {
      await commitCode(timeGapInMinutes, logDir);
    }, timeGapInMinutes * 60 * 1e3);
  });
  ...
  const stopDisposable = vscode.commands.registerCommand("codePusher.stopCommittingCode", async () => {
    ...
    if (push?.toLowerCase() === "yes" || push?.toLowerCase() === "y") {
      const remote = vscode.workspace.getConfiguration().get("codePusher.remoteName") || "origin";
      if (!remote) {
        vscode.window.showErrorMessage("Please provide the upstream name");
        return;
      }
      const branch = vscode.workspace.getConfiguration().get("codePusher.branchName") || "master";
      if (!branch) {
